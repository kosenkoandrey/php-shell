    ...
    <body data-ma-header="teal">
        ...
        <section id="main">
            ...
            <section id="content">
                <div class="container">
                    <div class="card">
                        <form id="update-tunnel" class="form-horizontal" role="form">
                            <input type="hidden" id="id" name="id">
                            <input type="hidden" id="factors" name="factors">
                            ...
                            <div class="card-body card-padding">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-6">
                                        <div class="fg-line">
                                            <textarea name="description" id="description" class="form-control"><?= htmlspecialchars($data['tunnel']['description']) ?></textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group">
        ...
        <script>
            $(document).ready(function() {
                $('#id').val('<?= APP::Module('Routing')->get['tunnel_id_hash'] ?>');
                $('#factors').val('<?= $data['tunnel']['factors'] ?>');
                $('#style').val('<?= $data['tunnel']['style'] ?>');
                $('#type').val('<?= $data['tunnel']['type'] ?>');
                $('#state').val('<?= $data['tunnel']['state'] ?>');
                $('#name').val('<?= $data['tunnel']['name'] ?>');

                autosize($('#description'));
                
                $('#update-tunnel').submit(function(event) {
                    ...
                    $.ajax({
                        type: 'post',
                        url: '<?= APP::Module('Routing')->root ?>admin/tunnels/api/update.json',
                        data: $(this).serialize(),
                        success: function(result) {
                            switch(result.status) {
                                case 'success':
                                    swal({
                                        title: 'Done!',
                                        text: 'Tunnel "' + name.val() + '" has been updated',
                                        type: 'success',
                            ...
                            $('#update-tunnel').find('[type="submit"]').html('Save changes').attr('disabled', false);
                        }
                    });
                  });
            });
