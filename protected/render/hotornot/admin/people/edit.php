    ...
    <body data-ma-header="teal">
        ...
        <section id="main">
            ...
            <section id="content">
                <div class="container">
                    <div class="card">
                        <form id="update-people" class="form-horizontal" role="form">
                            <input type="hidden" name="id" value="<?= APP::Module('Routing')->get['people_id_hash'] ?>">
                            ...
                            <div class="card-body card-padding">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <div class="fg-line">
                                            <textarea name="description" id="description" class="form-control"><?= $data['people']['description'] ?></textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group">
        ...
        <script>
            $(document).ready(function() {
                ...
                $('#city').val('<?= $data['people']['city'] ?>');
                $('#image').val('<?= $data['people']['image'] ?>');
		...
		$image_crop = $('#upload-demo').croppie({
                ...
                $('#update-people').submit(function(event) {
                    var form = this;
			...
			size: 'viewport'
                        ...
                        $.ajax({
                            type: 'post',
                            url: '<?= APP::Module('Routing')->root ?>admin/hotornot/people/api/update.json',
                            data: $(form).serialize(),
                            success: function(result) {
                                switch(result.status) {
                                    case 'success':
                                        swal({
                                            title: 'Done!',
                                            text: 'People "' + name.val() + '" has been updated',
                                            type: 'success',
                                ...
                                $('#update-people').find('[type="submit"]').html('Save changes').attr('disabled', false);
                            }
                        });
                    });
                });
