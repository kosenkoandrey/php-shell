    ...
    <body data-ma-header="teal">
        ...
        <section id="main">
            ...
            <section id="content">
                <div class="container">
                    <div class="card">
                        <form id="edit-trigger" class="form-horizontal" role="form">
                            <input type="hidden" name="trigger" value="<?= APP::Module('Routing')->get['trigger_id_hash'] ?>">
        ...
        <script>
            $(document).ready(function() {
                $('#sub_id').val('<?= $data['trigger']['sub_id'] ?>');
                $('#target_0').val('<?= $data['trigger']['target'][0] ?>');
                $('#target_1').val('<?= $data['trigger']['target'][1] ?>');
                ...
                $('#edit-trigger').submit(function(event) {
                    ...
                    $.ajax({
                        type: 'post',
                        url: '<?= APP::Module('Routing')->root ?>admin/triggers/api/update.json',
                        data: $(this).serialize(),
                            ...
                            switch(result.status) {
                                ...
                                case 'error': 
                                    $.each(result.errors, function(i, error) {
                                        switch(error) {
                                            case 1: $('#sub_id').addClass('has-error').nextAll('.error').eq(0).addClass('is-visible').html('Not found'); break;
                                            case 2: $('#sub_id').addClass('has-error').nextAll('.error').eq(0).addClass('is-visible').html('Not specified'); break;
                                            case 3: $('#target_0').addClass('has-error').nextAll('.error').eq(0).addClass('is-visible').html('Not specified'); break;
                                            case 4: $('#target_1').addClass('has-error').nextAll('.error').eq(0).addClass('is-visible').html('Not specified'); break;
                                        }
                                    });
                                    break;
                            }

                            $('#edit-trigger').find('[type="submit"]').html('Save changes').attr('disabled', false);
                        }
                    });
                  });
            });
