    ...
    <body data-ma-header="teal">
        ...
        <section id="main">
            ...
            <section id="content">
                <div class="container">
                    <div class="card">
                        <form id="edit-article" class="form-horizontal" role="form">
                            <input type="hidden" name="id" value="<?= APP::Module('Routing')->get['article_id_hash'] ?>">
                            ...
                            <div class="card-body card-padding">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <div class="fg-line">
                                            <textarea name="annotation" id="annotation" class="form-control"><?= $data['article']['annotation'] ?></textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <select name="tags[]" id="tags" data-placeholder="Set tags..." multiple style="width: 100%">
                                            <? foreach ((array) explode(',', $data['article']['tags']) as $tag) { ?><option value="<?= $tag ?>" selected><?= $tag ?></option><? } ?>
                                        </select>
                                    </div>
                                </div>
                                <div class="form-group">
        ...
        <script>
            $(document).ready(function() {
                ...
                $('#uri').val('<?= $data['article']['uri'] ?>');
                $('#page_title').val('<?= $data['article']['page_title'] ?>');
                $('#h1_title').val('<?= $data['article']['h1_title'] ?>');
                $('#image').val('<?= $data['article']['image'] ?>');
                $('#description').val('<?= $data['article']['description'] ?>');
		...
		$('#upload-image').on('change', function() { 
                ...
                $('#edit-article').submit(function(event) {
                    var form = this;
			...
			size: 'viewport'
                        ...
                        $.ajax({
                            type: 'post',
                            url: '<?= APP::Module('Routing')->root ?>admin/blog/api/articles/update.json',
                            data: $(form).serialize(),
                            success: function(result) {
                                switch(result.status) {
                                    case 'success':
                                        swal({
                                            title: 'Done!',
                                            text: 'Article "' + page_title.val() + '" has been updated',
                                            type: 'success',
                                ...
                                $('#edit-article').find('[type="submit"]').html('Save changes').attr('disabled', false);
                            }
                        });
                    });
                });
