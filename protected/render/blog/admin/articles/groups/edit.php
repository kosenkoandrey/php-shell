    ...
    <body data-ma-header="teal">
        ...
        <section id="main">
            ...
            <section id="content">
                <div class="container">
                    <div class="card">
                        <form id="edit-group" class="form-horizontal" role="form" enctype="multipart/form-data">
                            <input type="hidden" name="id" value="<?= APP::Module('Routing')->get['group_id_hash'] ?>">
                            
                            <div class="card-header">
                                <h2>Edit article group</h2>
                            </div>
                            ...
                            <div class="card-body card-padding">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <div class="fg-line">
                                            <textarea name="annotation" id="annotation" class="form-control"><?= $data['group']['annotation'] ?></textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <img src="<?= $data['group']['image'] ?>" class="thumbnail" width="100%">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <div class="fg-line">
                                            <textarea name="description" id="description" class="form-control"><?= $data['group']['description'] ?></textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group">
        ...
        <script>
            $(document).ready(function() {
                ...
                $('#uri').val('<?= $data['group']['uri'] ?>');
                $('#page_title').val('<?= $data['group']['page_title'] ?>');
                $('#h1_title').val('<?= $data['group']['h1_title'] ?>');
                $('#keywords').val('<?= $data['group']['keywords'] ?>');
                $('#robots').val('<?= $data['group']['robots'] ?>');
                ...
                $('#edit-group').submit(function(event) {
                    ...
                    $.ajax({
                        type: 'post',
                        url: '<?= APP::Module('Routing')->root ?>admin/blog/api/articles/groups/update.json',
                        data: form_data,
                        cache: false,
                        processData: false,
                        contentType: false,
                        success: function(result) {
                            switch(result.status) {
                                case 'success':
                                    swal({
                                        title: 'Done!',
                                        text: 'Group "' + page_title.val() + '" has been updated',
                                        type: 'success',
                            ...
                            $('#edit-group').find('[type="submit"]').html('Save changes').attr('disabled', false);
                        }
                    });
                  });
            });
