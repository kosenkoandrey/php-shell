...
?>
<button id="<?= $like_uniqid ?>" data-token="<?= APP::Module('Crypt')->Encode(json_encode(['type' => $data['type'], 'id' => $data['id']])) ?>" class="btn-like btn <?= $like['state'] ? 'btn-danger' : 'btn-default' ?> bg waves-effect <? if (isset($data['class'])) { echo implode(' ', $data['class']); } ?>"><i class="fa fa-<?= $like['state'] ? 'heart' : 'heart-o' ?>"></i> <?= $data['text'] ?> <sup><?= $like['count'] ?></sup></button>
<? 
    ...
    $(document).ready(function() {
        $(document).on('click', '.btn-like', function() {
            ...
            if (APP::Module('Users')->user['id']) {
                ?>
                $(this).find('sup').html(parseInt($(this).find('sup').text()) + ($(this).hasClass('btn-default') ? 1 : -1));
