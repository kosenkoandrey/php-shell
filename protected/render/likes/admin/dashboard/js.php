...
?>
<script>
    function GetLikes(nav) {
        $('#likes-period > button').removeAttr('disabled');
        if (nav) $('#likes-period > button[data-period="' + nav + '"]').attr('disabled', 'disabled');
        $('#likes-chart').html('<div class="text-center"><div class="preloader pl-xxl"><svg class="pl-circular" viewBox="25 25 50 50"><circle class="plc-path" cx="50" cy="50" r="20" /></svg></div></div>');
        
        $.ajax({
            url: '<?= APP::Module('Routing')->root ?>admin/likes/api/dashboard.json',
            data: {
                date: {
                    from: $('#likes-date-from').val(),
                    to: $('#likes-date-to').val()
                }
            },
            type: 'POST',
            dataType: 'json',
		...
		$("#likes-chart").bind("plothover", function (event, pos, item) {
                    if (item) {
    ...
    $(document).on('click', "#likes-period > button",function() {
        ...
        $('#likes-date-to').val(to);
        $('#likes-date-from').val(from);

        $('#likes-calendar-from').html(from_date.getDate() + '.' + (from_date.getMonth() + 1) + '.' + from_date.getFullYear());
        $('#likes-calendar-to').html(to_date.getDate() + '.' + (to_date.getMonth() + 1) + '.' + to_date.getFullYear());

        GetLikes(period);
    }); 

    $('#likes-calendar').popover({
        html: true,
        content: [
    ...
    $(document).on('click', '#tab-nav-<?= $data['hash'] ?> > a',function() {
        $('#likes-period > button[data-period="1 months"]').trigger('click');
    });
</script>