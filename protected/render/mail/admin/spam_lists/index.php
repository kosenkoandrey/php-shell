...
<!--[if IE 9 ]><html class="ie9"><![endif]-->
    <head>
        ...
        <title>PHP-shell - IP spam lists</title>
        ...
        <style>
            #ip-table-header .actionBar .actions > button {
                display: none;
            }
        </style>
    ...
    <body data-ma-header="teal">
                            ...
                            <ul class="actions">
                                <li class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right">
                                        <li><a href="<?= APP::Module('Routing')->root ?>admin/mail/spam_lists/ip/add">Add IP addr</a></li>
                                    </ul>
                                </li>
                            </ul>
                        </div>
                        <div class="card-body">
                            <table class="table table-hover table-vmiddle" id="ip-table">
                                <thead>
                                    <tr>
                                        <th data-column-id="id" data-visible="false" data-type="numeric" data-order="desc">ID</th>
                                        <th data-column-id="addr">IP</th>
                                        <th data-column-id="spam_lists" data-formatter="spam_lists">Spam lists</th>
        ...
        <script>
            $(document).ready(function() {
                var ip_table = $("#ip-table").bootgrid({
                    ajax: true,
                    ajaxSettings: {
                        method: 'POST',
                        cache: false
                    },
                    url: '<?= APP::Module('Routing')->root ?>admin/mail/api/spam_lists/ip/get.json',
                    css: {
                        icon: 'zmdi icon',
                    ...
                    formatters: {
                        spam_lists: function(column, row) {
                    ...
                    ip_table.find('.remove-ip').on('click', function (e) {
                        var ip_id = $(this).data('ip-id');
                        ...
                        swal({
                            ...
                            if (isConfirm) {
                                $.post('<?= APP::Module('Routing')->root ?>admin/mail/api/spam_lists/ip/remove.json', {
                                    id: ip_id
                                }, function() { 
                                    ip_table.bootgrid('reload', true);
                                    swal('Deleted!', 'IP has been deleted', 'success');
                                });
                            }
                        });
                    });
