...
<!--[if IE 9 ]><html class="ie9"><![endif]-->
    <head>
        ...
        <style>
            #transport-table-header .actionBar .actions > button {
                display: none;
            }
        </style>
    ...
    <body data-ma-header="teal">
                            ...
                            <ul class="actions">
                                <li class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right">
                                        <li><a href="<?= APP::Module('Routing')->root ?>admin/mail/transport/add">Add Transport</a></li>
                                    </ul>
                                </li>
                            </ul>
                        </div>
        ...
        <script>
            $(document).ready(function() {
                var transport_table = $("#transport-table").bootgrid({
                    ajax: true,
                    ajaxSettings: {
                        method: 'POST',
                        cache: false
                    },
                    url: '<?= APP::Module('Routing')->root ?>admin/mail/api/transport/list.json',
                    css: {
                        icon: 'zmdi icon',
                    ...
                    transport_table.find('.remove-transport').on('click', function (e) {
                        var connection_id = $(this).data('transport-id');

                        swal({
                            title: 'Are you sure?',
                            text: 'You will not be able to recover this transport method',
                            type: 'warning',
                            ...
                            if (isConfirm) {
                                $.post('<?= APP::Module('Routing')->root ?>admin/mail/api/transport/remove.json', {
                                    id: connection_id
                                }, function() { 
                                    transport_table.bootgrid('reload', true);
                                    swal('Deleted!', 'Transport method has been deleted', 'success');
                                });
                            }
                        });
                    });
