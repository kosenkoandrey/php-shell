    ...
    <body data-ma-header="teal">
        ...
        <section id="main">
            ...
            <section id="content">
                <div class="container">
                    <div class="card">
                        <form id="update-quiz" class="form-horizontal" role="form">
                            ...
                            <div class="card-body card-padding">
                                ...
                                <div class="form-group">
                                    <div class="col-sm-offset-2 col-sm-5">
                                        <button type="submit" class="btn palette-Teal bg waves-effect btn-lg">Edit</button>
                                    </div>
                                </div>
                            </div>
                        </form>
        ...
        <script>
            $(document).ready(function() {
                $('#id').val('<?= APP::Module('Routing')->get['question_id_hash'] ?>');
                $('#text').val('<?= $data['text'] ?>');

                autosize($('#text'));
                
                $('#update-quiz').submit(function(event) {
                    ...
                    $.ajax({
                        type: 'post',
                        url: '<?= APP::Module('Routing')->root ?>admin/quiz/api/question/update.json',
                        data: $(this).serialize(),
                        success: function(result) {
                            switch(result.status) {
                                case 'success':
                                    swal({
                                        ...
                                        window.location.href = '<?= APP::Module('Routing')->root ?>admin/quiz/question';
                                    });
                                    break;
                                case 'error': 
                            ...
                            $('#update-question').find('[type="submit"]').html('Save changes').attr('disabled', false);
                        }
                    });
                  });
            });
