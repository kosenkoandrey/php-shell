...
class Billing {
    ...
    public function AddProduct() {
        APP::Render(
												'billing/admin/products/add',
												'include',
												[
                'products' => APP::Module('DB')->Select(
                    $this->settings['module_billing_db_connection'], ['fetchAll', PDO::FETCH_ASSOC],
                    ['name', 'amount', 'access_link', 'descr_link', 'members_access', 'related_products', 'id'], 'billing_products'
                ),
            ]);
    }
    ...
    public function EditProduct() {
        $product_id = APP::Module('Crypt')->Decode(APP::Module('Routing')->get['product_id_hash']);
								...
								$product = APP::Module('DB')->Select(
												...
												['name', 'amount', 'access_link', 'descr_link', 'members_access', 'related_products', 'plus_products'], 'billing_products',
    ...
    public function APIUpdateSettings() {
            ...
            'user_id' => $user_id,
				...
				public function AddRelatedProducts($user_id, $products){
								...
								foreach	($products as $product){
												$out['access_id'][] = APP::Module('DB')->Insert(
																$this->settings['module_members_db_connection'], 'related_products',
																Array(
																				'id' => 'NULL',
																				'user_id' => [$user_id, PDO::PARAM_INT],
            ...
            'user_id' => $user_id,
				...
				public function AddPlusProducts($data){
								APP::Module('DB')->Insert(
												$this->settings['module_members_db_connection'], 'billing_invoices_products',
												Array(
																'id' => 'NULL',
        ...
        foreach (APP::Module('DB')->Select(
            ...
            $products_list[$product['id']] = [
                ...
                'amount' => $product['amount']
								...
								$products = APP::Module('DB')->Select(
												$this->settings['module_billing_db_connection'], ['fetchAll', PDO::FETCH_ASSOC],
												['product_id', 'price', 'id'], 'billing_invoices_products',
												[['invoice_id', '=', $invoice_id, PDO::PARAM_INT]]
								);
        ...
        $products_counter = APP::Module('DB')->Select(
												...
												['count(*)'], 'billing_invoices_products'
								);
        ...
        APP::Render(
            'billing/admin/invoices/edit',
                ...
                'invoice' => [
                    ...
                    'main' => APP::Module('DB')->Select(
																								$this->settings['module_billing_db_connection'], ['fetch', PDO::FETCH_ASSOC],
																								['user_id', 'state', 'amount', 'id', 'UNIX_TIMESTAMP(up_date) as up_date', 'UNIX_TIMESTAMP(cr_date) as cr_date'], 'billing_invoices',
            ...
            APP::Module('Billing')->settings['module_billing_db_connection'],
            ['fetchAll', PDO::FETCH_ASSOC],
            ['billing_products.id', 'billing_products.plus_products'], 'billing_products',
            [['billing_products.id', 'IN', $products_id, PDO::PARAM_STR]]
        ) as $product){
												...
												if(count($plus_products)){
																foreach	($plus_products as $item){
																				APP::Module('TaskManager')->Add(
																								'Billing',
																								'AddPlusProducts',
																								date('Y-m-d H:i:s', strtotime($item['time'])),
																								json_encode([$invoice_id, $item['id'], 0]),
																								'plus_products',
																								'wait'
																				);
																}
        ...
        foreach (APP::Module('DB')->Select(
            $this->settings['module_billing_db_connection'], ['fetchAll', PDO::FETCH_ASSOC],
            ['id', 'user_id', 'amount', 'author', 'state', 'up_date'], 'billing_invoices',
            false,
            false, false, false,
            [$request['sort_by'], $request['sort_direction']],
            [($request['current'] - 1) * $request['rows'], $request['rows']]
        ...
        exit;
            ...
            ['COUNT(id)'], 'users',
            [['id', '=', $_POST['invoice']['user_id'], PDO::PARAM_INT]]
        )) {
            $out['status'] = 'error';
            $out['errors'][] = 1;
        }
        ...
        // Update invoice
								APP::Module('DB')->Update($this->settings['module_billing_db_connection'], 'billing_invoices', [
        ...
        if($_POST['invoice']['state'] == 'success'){
        ...
        exit;
            ...
            ['users.email', 'billing_invoices.id', 'billing_invoices.amount', 'billing_invoices.state', 'billing_invoices.author','billing_invoices.user_id'], 'billing_invoices',
...
class ProductsSearch {
        ...
        return APP::Module('DB')->Select(
            ...
            ['id'], 'billing_products',
            [['id', $settings['logic'], $settings['value'], PDO::PARAM_INT]]
        );
    }
